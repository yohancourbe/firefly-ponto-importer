...
export class SyncPonto {
  ...
  private async syncAccounts(): Promise<AccountPair[]> {
    ...
    for (const pontoAccount of pontoAccounts) {
      const fireflyAccount = fireflyAccounts.find(
        (fireflyAccount) => fireflyAccount.attributes.account_number === pontoAccount.attributes.reference
      );
  ...
  private async syncTransactions(account: AccountPair, allAccounts: AccountPair[]) {
    ...
    this.logging.info(
      `Syncing transactions for account ${account.ponto.attributes.reference} from ${lastSyncedTransactionId || "beginning"}`
    );
    ...
    for (const pontoTransaction of pontoTransactions.sort().reverse()) {
      ...
      if (knownCounterpartReference) {
        ...
        newTransaction.destination_id = account.firefly.id;
        newTransaction.source_name = `${pontoTransaction.attributes.counterpartName} (${
          pontoTransaction.attributes.counterpartReference || "unknown reference"
        })`;
      } else {
        this.logging.info(`Creating withdrawal transaction`);
        newTransaction.type = FireflyTransactionType.Withdrawal;
        newTransaction.source_id = account.firefly.id;
        newTransaction.destination_name = `${pontoTransaction.attributes.counterpartName} (${
          pontoTransaction.attributes.counterpartReference || "unknown reference"
        })`;
      }
    ...
    if (!lastSyncedTransactionId) {
      ...
      await this.fireflyAccountsApi.update(account.firefly.id, {
